...
pub(crate) fn assert_vault_conservation_invariants(
    ...
    let vc = borsh::from_slice::<VaultConfig>(
        &state.get_account_by_id(vault.vault_config_account_id).data,
    )
    .expect("vault config");
    ...
    for stream_id in 0u64..vc.next_stream_id {
        ...
        let Ok(sc) = borsh::from_slice::<StreamConfig>(data) else {
            continue;
        };
        sum = sum
...
pub(crate) fn force_stream_state_closed(state: &mut V03State, stream_pda: AccountId) {
    ...
    stream_account.data =
        Data::try_from(borsh::to_vec(&stream_cfg).unwrap()).expect("stream payload fits");
