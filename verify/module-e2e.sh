...
#
# Usage:
#   CHAIN=local   ./verify/module-e2e.sh   (default)
#   CHAIN=testnet ./verify/module-e2e.sh
#   ./verify/module-e2e.sh --verbosity quiet|normal|verbose
# Driven by: MODE=module ./verify/e2e.sh <chain> run
...
source "$REPO_ROOT/verify/lib/chain_poll.sh"
...
if ps_is_testnet; then
  ...
  [[ -n "$FIXTURE" ]] || FIXTURE="$REPO_ROOT/verify/fixtures/testnet-module.json"
  [[ -f "$FIXTURE" ]] || FIXTURE="$REPO_ROOT/verify/fixtures/testnet.json"
  if [[ ! -f "$FIXTURE" ]]; then
...
[[ -d "$MODULES" ]] || {
  ...
  narr_hint "Run: MODE=module ./verify/e2e.sh local prepare"
...
# auth_transfer_init — see verify/lib/auth_transfer.sh (ps_auth_transfer_ensure).
...
# known shapes (mirrors run_e2e.py chain_action_tx_hash). Prints empty on
...
if ps_is_testnet; then
  ...
  if ps_is_owner_privacy_e2e; then
    ...
    if [[ "${MODULE_E2E_SKIP_FUND:-0}" == "1" ]]; then
      ...
      narr_hint "Pre-fund funder with ./verify/testnet/fund-testnet-accounts.sh before the demo"
    else
    ...
    narr_hint "Pre-fund with ./verify/testnet/fund-testnet-accounts.sh before the demo"
    if ps_is_provider_privacy_e2e; then
      OWNER_HEX="$(account_id_to_hex "$OWNER")"
      [[ -z "$OWNER_HEX" || -z "${PROVIDER_HEX:-}" ]] && ps_fatal "could not resolve hex ids for provider dust pre-shield"
      narr_step "Dust pre-shielding private provider from public owner"
