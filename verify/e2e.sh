...
# Usage: ./verify/e2e.sh <local|testnet> <run|prepare|teardown|build>
#
# Commands:
...
cmd_prepare_local() {
  ...
  if ps_is_module_mode; then
    "$REPO_ROOT/verify/lifecycle.sh" localnet ensure || return 1
...
PY
  ...
  if [[ "${FULL_RESET:-0}" == "1" ]]; then
    ...
    "$REPO_ROOT/verify/fixture.sh" prefund
    "$REPO_ROOT/verify/lifecycle.sh" snapshot save "$snapshot_name"
  elif "$REPO_ROOT/verify/lifecycle.sh" snapshot validate "$snapshot_name" 2>/dev/null; then
...
cmd_prepare_testnet() {
  ...
  "$REPO_ROOT/verify/lifecycle.sh" testnet wallet-ensure
...
cmd_run() {
  ...
  if ps_is_module_mode; then
    ...
    MODULES="$MODULES_USER" ARTIFACT="$ARTIFACT" \
      ...
      "$REPO_ROOT/verify/module-e2e.sh" ${E2E_VERBOSITY:+--verbosity "$E2E_VERBOSITY"} || {
  ...
  python3 "$REPO_ROOT/verify/store/run_e2e.py" \
    --repo "$REPO_ROOT" \
    --phase "$E2E_PHASE" \
    --artifact "$ARTIFACT" \
...
cmd_teardown() {
  ...
  if ps_is_local && [[ "${STOP_LOCALNET:-0}" == "1" ]]; then
    ...
    "$REPO_ROOT/verify/lifecycle.sh" localnet stop
  fi
