...
# AT ensure for E2E: prefer verify/lib/auth-transfer-ensure.sh (Step 32); bootstrap
...
lez_testnet_submit_bin() {
  ...
  local built="$REPO_ROOT/verify/testnet/submit/target/release/lez-testnet-submit"
  if [[ -x "$built" ]]; then
    echo "$built"
    return 0
  fi
  echo "ERROR: build lez-testnet-submit (cd verify/testnet/submit && cargo build --release)" >&2
  return 1
}
...
testnet_owner_balance() {
  ...
  python3 "$REPO_ROOT/verify/testnet/testnet_rpc.py" account-balance "$owner" 2>/dev/null || echo "0"
}
...
testnet_rpc_last_block() {
  python3 "$REPO_ROOT/verify/testnet/testnet_rpc.py" block-height
}
