...
MANIFEST="${FIXTURE_MANIFEST:-$REPO_ROOT/verify/fixtures/testnet-module.json}"
...
OWNER=""
PROVIDER=""
if [[ -f "$REPO_ROOT/verify/fixtures/testnet.json" && "${TESTNET_REUSE_FIXTURE:-1}" == "1" ]]; then
  OWNER="$(python3 -c "import json; print(json.load(open('$REPO_ROOT/verify/fixtures/testnet.json')).get('owner_account_id','').strip())")"
  PROVIDER="$(python3 -c "import json; print(json.load(open('$REPO_ROOT/verify/fixtures/testnet.json')).get('provider_account_id','').strip())")"
  if [[ -n "$OWNER" ]]; then
...
cargo run --quiet --manifest-path "$REPO_ROOT/verify/seed/Cargo.toml" --bin bootstrap_testnet_fixture -- \
