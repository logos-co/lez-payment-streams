...
user_journey_require_shell() {
  if [[ "${USER_JOURNEY_SHELL:-}" != 1 ]]; then
    echo "Run this from the toolchain shell: ./verify/repro-shell.sh" >&2
    exit 1
  fi
}
...
user_journey_require_tools() {
  ...
  if ((${#missing[@]} > 0)); then
    echo "Missing on PATH: ${missing[*]}. Use ./verify/repro-shell.sh" >&2
    exit 1
  fi
}
...
user_journey_default_fixture_manifest() {
  local root
  root="$(user_journey_repo_root)"
  echo "${root}/verify/fixtures/testnet-module.json"
}
