...
export FIXTURE_MANIFEST="${FIXTURE_MANIFEST:-$REPO_ROOT/verify/fixtures/localnet.json}"
...
owner="$("$REPO_ROOT/verify/fixture.sh" vault resolve-id 2>/dev/null | head -1 || true)"
...
if "$REPO_ROOT/verify/lifecycle.sh" localnet status 2>/dev/null | grep -q running; then
  high_id=99999
  if "$REPO_ROOT/verify/fixture.sh" vault config-is-empty "$high_id"; then
...
resolved="$("$REPO_ROOT/verify/fixture.sh" vault resolve-id)"
