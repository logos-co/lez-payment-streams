...
# Usage: ./verify/lifecycle.sh <localnet|testnet|snapshot> <command> [args]
...
cmd_snapshot_restore() {
  ...
  local f
  for f in .lez_payment_streams-state .lez_payment_streams-fixture-provider; do
    [[ -f "$snap_dir/$f" ]] && mkdir -p "$REPO_ROOT/.scaffold" && cp -a "$snap_dir/$f" "$REPO_ROOT/.scaffold/$f"
  done
  ...
  rm -f "$REPO_ROOT/verify/fixtures/localnet.json"
