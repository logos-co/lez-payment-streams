...
exec nix shell --accept-flake-config \
  ...
  --command bash -c "export USER_JOURNEY_SHELL=1; export REPO=\"${REPO_ROOT}\"; export FIXTURE_MANIFEST=\"${REPO_ROOT}/verify/fixtures/testnet-module.json\"; export PATH=\"${CARGO_BIN}:\${PATH}\"; cd \"${REPO_ROOT}\" && echo 'USER_JOURNEY shell: logoscore + lgpm (dev module variants). Optional reset: ./verify/repro-reset.sh' && exec bash"
