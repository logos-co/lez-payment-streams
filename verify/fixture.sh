...
# Usage: ./verify/fixture.sh <command> [args]
#
# Commands:
...
wait_clock_synced() {
  ...
  if cargo run -q --manifest-path "$REPO_ROOT/verify/seed/Cargo.toml" \
...
#
# Uses the pinned LEZ wallet on PATH (see make deploy / verify/lib/common.sh).
fund_owner_account() {
...
cmd_prefund() {
  ...
  if [[ -f "$REPO_ROOT/.scaffold/.lez_payment_streams-state" ]]; then
    ...
    source "$REPO_ROOT/.scaffold/.lez_payment_streams-state"
    owner="${SIGNER_ID:-}"
  fi
  ...
  cargo run -q --manifest-path "$REPO_ROOT/verify/seed/Cargo.toml" \
...
resolve_owner() {
  ...
  if ! ps_is_testnet && [[ -f "$REPO_ROOT/.scaffold/.lez_payment_streams-state" ]]; then
    owner="$(grep '^SIGNER_ID=' "$REPO_ROOT/.scaffold/.lez_payment_streams-state" 2>/dev/null |
      cut -d= -f2 | tr -d '"'\''')"
  fi
  if [[ -z "$owner" ]]; then
...
# shellcheck source=verify/lib/vault_scan.sh
source "$REPO_ROOT/verify/lib/vault_scan.sh"
...
cmd_manifest_write() {
  ...
  [[ -f "$REPO_ROOT/.scaffold/.lez_payment_streams-fixture-provider" ]] ||
    ps_fatal "Missing provider marker (.lez_payment_streams-fixture-provider)"
  provider="$(cat "$REPO_ROOT/.scaffold/.lez_payment_streams-fixture-provider")"
  manifest="${FIXTURE_MANIFEST:-$REPO_ROOT/verify/fixtures/localnet.json}"
...
Environment:
  FIXTURE_MANIFEST    — Path to fixture JSON (default: verify/fixtures/localnet.json)
