...
Cross-step APIs and wire shapes. Normative detail: [integration-decisions.md](decisions.md)
...
Operator commands: [verification-matrix.md](matrix.md) and pillar runbooks under [reproduce/store.md](../reproduce/store.md) and [payment-streams-module/](../../module/). Historical step runbooks live under [archive/completed-steps-index.md](../archive/completed-steps-index.md).
...
- MVP hooks are synchronous blocking C function pointers at the liblogosdelivery boundary ([N3](decisions.md#n3-provider-side-verification-latency-and-blocking-hooks))
...
- Step 16 bridge policy: [N3a](decisions.md#n3a-step-16-threading--approach-a-experiment-2025-06-18),
  [N3b](decisions.md#n3b-step-16-hook-registration-lifecycle-2025-06-18),
  [N3c](decisions.md#n3c-inbound-missing-proof-null-proof_hex-2025-06-18),
...
completion event ([N3a](decisions.md#n3a-step-16-threading--approach-a-experiment-2025-06-18)).
...
`payment_streams_module_impl.h` or `lm methods` omits them ([N11](decisions.md#n11-universal-module-public-api)).
...
cargo run -q --manifest-path verify/seed/Cargo.toml --bin seed_localnet_fixture -- \
...
- Missing proof on inbound Store: Step 16 passes empty `proofBytes`; paid demo expects a
  verdict failure, not OK ([N3c](decisions.md#n3c-inbound-missing-proof-null-proof_hex-2025-06-18))
- Step 17 demo: provider runs paid-only (verifier always on); users learn provider PeerId
  off-band ([store-eligibility.md](../reproduce/store.md))

## Fixture and provider binding

- Default manifest: `verify/fixtures/localnet.json` (`FIXTURE_MANIFEST`)
...
- Runtime id: `logos_execution_zone` ([D4](decisions.md#d4-wallet-module-runtime-name))
...
Full specification: [N8](decisions.md#n8-canonical-store-request-bytes-format).
