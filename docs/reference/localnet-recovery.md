# Local demo recovery policy

Local LEZ demos use fake funds and a disposable chain. Prefer a known-good fixture over
debugging stale state. Public testnet operators should not follow this reset-first policy;
see Step 12 runbook testnet notes in [`archive/steps/user-eligibility-runbook.md`](../archive/steps/user-eligibility-runbook.md).

Related: [`archive/steps/local-chain-fixture-handoff.md`](../archive/steps/local-chain-fixture-handoff.md),
[`archive/steps/local-chain-fixture.md`](../archive/steps/local-chain-fixture.md).

## Principle

- Treat on-chain localnet state and `verify/fixtures/localnet.json` as disposable.
- Treat `payment_streams_state.json` (logoscore `instancePersistencePath`) as disposable
  for every demo run unless you intentionally continue one session.
- Keep wallet keys and installed `.lgx` modules across resets when they still work.
- When unsure, or when verification fails, reset the chain and re-seed instead of tuning
  demo-only env bypasses.

After a guest ImageID cut, treat private notes in cloned wallets as burned.
Derive a fresh private owner/provider for the next privacy cell; do not shield a note
whose nullifier was spent under the previous program.
Burned ids: `.scaffold/e2e/burned-private-ids.json`.

`PAYMENT_STREAMS_ALLOW_DEPLETED_STREAM_PROOF` is an escape hatch when chain writes are broken
but you still need to show proof bytes. It is not the normal local demo path.

## Demo modes

| Mode | Goal | Chain | Module off-chain state |
| --- | --- | --- | --- |
| Smoke | Mapping, reads, `stream_proof` on a **per-run** stream id | Step 17b: vault-only prepare; E2E or Step 12 script creates at `next_stream_id` | New `--persistence-path` each run |
| Full arc | Proposal, `createStream`, proof; Step 13 prepare → verify | Same prepare path; `FULL_RESET=1` rebuilds snapshot | Always wipe before the arc |

Default reuse (Step 17b + 24c): restore funded **vault-only** baseline from
`.scaffold/snapshots/funded/`; each demo run creates a new stream at `next_stream_id` and teardown
closes it (provider-signed close locally). Full pinata rebuild: `make full-reset-localnet`
(`FULL_RESET=1 ./verify/e2e.sh local prepare`).

## What to keep vs wipe

Keep across local resets (slow or stable):

- Patched `logos_execution_zone` and `payment_streams_module` under `MODULES`.
- `PAYMENT_STREAMS_GUEST_BIN` pointing at the built guest `.bin`.
- `.scaffold/wallet/wallet_config.json` and valid `storage.json` when deploy and `open` work.
- Step 17b snapshot `.scaffold/snapshots/funded/` when `lez_pin` and guest ImageID unchanged
  (see [step-17b-localnet-snapshot-restore.md](../plan/completed/step-17b-localnet-snapshot-restore.md)).

Wipe on blank-slate recovery:

| Path | Reason |
| --- | --- |
| LEZ cache `…/repos/lez/<pin>/rocksdb/` | Authoritative ledger; replaced on restore or `FULL_RESET` |
| `.scaffold/state/` | Scaffold localnet bookkeeping (restore copies from snapshot) |
| `verify/fixtures/localnet.json` | Regenerated by seed; must match deployed program |
| `.scaffold/step12-persist*` | Stale peer mapping, session keys, inventory |
| `.scaffold/step13-persist*` | Stale provider acceptance rows for verify demos |
| `.scaffold/.lez_payment_streams-state` | Owner signer drift after wallet reinit |

Optional wipe on full blank slate:

| Path | Reason |
| --- | --- |
| `.scaffold/.lez_payment_streams-fixture-provider` | New provider line in manifest |
| `.scaffold/wallet/storage.json` | Invalid format (`missing field sk`, etc.) — use reinit |

## Recovery ladder

0. Step 17b fast path — `make prepare-localnet` (`verify/e2e.sh local prepare`, restore vault-only snapshot).
   Invalid snapshot or guest rebuild → `make full-reset-localnet` (`FULL_RESET=1 verify/e2e.sh local prepare`).
   Step 17 E2E creates the stream; do not expect stream `0` in the prepare manifest.
1. Soft — New logoscore run with a fresh `--persistence-path`, re-register provider mapping,
   check stream status via `listMyStreams` or reads.
2. Re-seed — Same chain: `make build deploy`, `./verify/seed/seed.sh`, clear
   module persist dirs, refresh manifest.
3. Blank slate — `make full-reset-localnet` (`FULL_RESET=1 verify/e2e.sh local prepare`)
   (rebuild vault-only snapshot: stop localnet, wipe ledger + `.scaffold/state/`, prefund, snapshot).
4. Run demo — `logoscore` with a new `--persistence-path`, `open` wallet, then Step 12 calls.

Signals to jump to step 3:

- `./verify/archive/verify-step10a-dod.sh` fails.
- `lgs localnet start` fails with missing `sequencer/service/configs/debug/sequencer_config.json`
  after Step 11d pin bump — run `./verify/lib/ensure-scaffold-lez-layout.sh` (links `sequencer` → `lez/sequencer` in the LEZ cache).
- `prepareEligibilityForStoreQuery` returns `STREAM_DEPLETED` on the current run’s stream after
  long idle time or wrong run order — run `make prepare-localnet` (clock sync runs on
  restore), tear down prior streams, or `make full-reset-localnet` if the vault sizing is wrong.
  Do not use `PAYMENT_STREAMS_ALLOW_DEPLETED_STREAM_PROOF` for normal local demos.
- `make deploy` or `storage.json` parse errors.
- Manifest program id stale after `make build`.

## One-shot blank slate

From repo root (unified prepare — replaces archived `demo-localnet-fresh.sh`):

```bash
make full-reset-localnet
```

Wallet storage broken — reinit wallet, then rebuild baseline:

```bash
./verify/archive/reinit-scaffold-wallet.sh
make full-reset-localnet
```

Skip Step 10a verify at the end of archived prepare (faster, after you trust seed): use
`make prepare-localnet`; the modern prepare path does not invoke `verify-step10a-dod.sh`.

Then Step 12 smoke (set `PAYMENT_STREAMS_GUEST_BIN` to the built guest `.bin`):

```bash
export PAYMENT_STREAMS_GUEST_BIN="$PWD/program/methods/guest/target/riscv32im-risc0-zkvm-elf/docker/lez_payment_streams.bin"
export PERSIST_DIR="$PWD/.scaffold/step12-persist-$(date +%s)"
./verify/archive/verify-step12-dod.sh
```

After a fresh reset, require honest `stream_proof` in DoD smoke (fail on depleted instead of skip):

```bash
REQUIRE_STREAM_PROOF=1 ./verify/archive/verify-step12-dod.sh
```

Or offline only:

```bash
VERIFY_LOGOSCORE=0 ./verify/archive/verify-step12-dod.sh
```

Step 13 provider cross-test (after fresh reset when stream `0` is not depleted):

```bash
export PERSIST_DIR="$PWD/.scaffold/step13-persist-$(date +%s)"
VERIFY_LOGOSCORE=1 ./verify/archive/verify-step13-dod.sh
```

See [`archive/steps/provider-eligibility-runbook.md`](../archive/steps/provider-eligibility-runbook.md).

## Scripts

| Script | Role |
| --- | --- |
| [`verify/e2e.sh`](../../verify/e2e.sh) | Prepare / run / teardown (`make prepare-localnet`, `make verify-store-local`) |
| [`verify/lifecycle.sh`](../../verify/lifecycle.sh) | Localnet, snapshot save/restore/validate |
| [`verify/fixture.sh`](../../verify/fixture.sh) | Prefund, vault ensure, manifest |
| `verify/archive/demo-localnet-fresh.sh` | Legacy wrapper → `make full-reset-localnet` |
| `verify/archive/demo-localnet-prepare.sh` | Legacy wrapper → `make prepare-localnet` |
| `verify/archive/clear-demo-module-persist.sh` | Remove known Step 12 logoscore persist dirs under `.scaffold/` |
| `verify/archive/reinit-scaffold-wallet.sh` | Recreate scaffold wallet storage (does not reset sequencer state alone) |
| `verify/lib/ensure-scaffold-lez-layout.sh` | Symlink `sequencer` for LEZ 510+ layout after `lgs setup` |
| `verify/seed/seed.sh` | Idempotent on-chain seed when chain is already up |
| `verify/archive/wait-clock-synced.sh` | After restore, wait for demo clock ≈ wall time (pinata nudge if idle) |
| `verify/archive/wait-chain-settle.sh` | Owner nonce settle before seed create/close |
| `verify/archive/demo-stream-teardown-localnet.sh` | Provider-signed close + vault-only manifest |

## Testnet contrast

On public testnet, persist manifest, vault, and operator wallet. Reset is exceptional.
Local policy does not apply there.
