# Step 15 — normative plan excerpt

Status: complete on `logos-delivery` branch `feat/payment-streams-store-eligibility`
(commit `e59319d8`). Verify in `logos-delivery`:

- `make liblogosdelivery`
- `make logosdelivery_eligibility_smoke`
...
Implementation layout (fork): `library/store_eligibility/` (C ABI + JSON store query),
`logos_delivery/waku/waku_store/eligibility_canonical.nim` (N8 wire),
`logos_delivery/waku/waku_store/eligibility_hooks.nim` (inbound wrapper).
`protocol.nim` only sets requester `PeerId` for the wrapper and preserves a non-zero handler
`statusCode` (eligibility failures use 400 from the wrapper, not from protocol logic).
...
with a wrapper (`eligibility_hooks.nim`) that:
...
Eligibility policy stays out of `client.nim`; `protocol.nim` only supplies the requester
`PeerId` to the wrapper and does not implement verification.
