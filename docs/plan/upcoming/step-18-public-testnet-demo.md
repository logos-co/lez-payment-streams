...
for the current tree is complete (see Verified org deploy below). Remaining Part B on
`feat/step18-public-testnet`: read smoke PASS, `make bootstrap-testnet`, `make verify-step18`,
and align scripts with rc3 `wallet deploy-program` plus jsonrpsee `lez-testnet-submit`. Step 20
testnet journey rows depend on Step 18 DoD. `make verify-step17` on `CHAIN=local` is unchanged.
