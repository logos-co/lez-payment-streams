...
- [ ] LEZ operational pin updated to upstream `v0.2.0` (`a58fbce2…`) across
  the exhaustive file list in "Pin bump" above; `grep -rl 27360cb7` sweep
  returns only the two archived DoD scripts
- [ ] Wallet wrapper `upstream` input moved from `refs/pull/19/head` (closed)
  to `main`; resolved SHA recorded in `feature-branch-pins.md`
- [ ] All four wallet patches (`wallet-qt-guest-elf-from-env`,
...
- [ ] `wallet-qt-send-generic-public-transaction-json.patch` rewritten against
...
- [ ] `buildGenericPublicPayloadJson` in `payment_streams_module_writes.cpp`
  emits `program_id_hex` (no `program_elf_hex` / `program_dependencies_hex`)
- [ ] `chainUsesTestnetSubmit()` always returns `false` with the deferred-
  removal comment; all writes route through `submitGenericPublicViaFfi`
- [ ] `tools/lez-testnet-submit/` compiles against v0.2.0 (kept in tree as
  fallback; not invoked from the module)
- [ ] `lgs setup` run against `a58fbce2…`; localnet snapshot rebuilt
  (`make full-reset-localnet`)
- [ ] `MODE=module CHAIN=local ./scripts/e2e.sh local run` passes (all phases,
  including `claim`)
- [ ] `MODE=store CHAIN=local ./scripts/e2e.sh local run` passes
- [ ] `make verify-step17-back-to-back` passes
- [ ] `nix build .#payment-streams-ffi` succeeds
- [ ] `make program-id` produces a `program_id_hex` for the rebuilt guest
- [ ] `feature-branch-pins.md` updated (v0.2.0 pin, wrapper ref move, Qt patch
  rewrite, helper retirement deferred, `verify-step11d-dod.sh` line removed
  from Verification commands)
- [ ] N16 amended (v0.2.0 operational pin, helper no longer dispatched,
  Phase 9 reframed)
- [ ] N10 amended (QList-unreliability rationale marked Legacy-specific;
  JSON wrapper now persists for RISC0-serde reason)
- [ ] D6 revisit condition noted as met; static-dependency migration
  deferred to [Step 30](step-30-static-dependency-migration.md)
- [ ] Step 18 completed packets get "Superseded by Step 26" banner
- [ ] Archived rc5-asserting DoD scripts (`verify-step11a-dod.sh`,
